...
const MessagePanel = ({ event, socket }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-gradient-to-b from-gray-50 to-gray-100 shadow-lg rounded-2xl overflow-hidden">
      ...
      {/* Header */}
      <div className="p-3 sm:p-4 border-b bg-green-700 text-white flex items-center justify-between">
        <h2 className="font-semibold text-base sm:text-lg truncate">
          {event.title || event.name || "Untitled Event"}
        </h2>
        <span className="text-xs sm:text-sm text-green-100">Coordinator Chat</span>
      </div>

      {/* Messages List */}
      <div className="flex-1 overflow-y-auto px-3 sm:px-4 py-3 sm:py-4 scrollbar-thin scrollbar-thumb-green-400 scrollbar-track-transparent">
        {messages.length === 0 ? (
          <div className="flex h-full justify-center items-center text-gray-500 italic text-sm sm:text-base">
          ...
          messages.map((msg, idx) => {
            ...
            return (
              ...
              >
                <div
                  className={`max-w-[80%] sm:max-w-[65%] px-3 sm:px-4 py-2 sm:py-2.5 rounded-2xl text-sm sm:text-base shadow-md ${
                    isSentByCoordinator
                      ? "bg-green-600 text-white rounded-br-none"
                      : "bg-white text-gray-800 border border-gray-200 rounded-bl-none"
                  }`}
                >
                  <p className="break-words leading-snug sm:leading-normal">{msg.content}</p>

                  <p
                    className={`text-[9px] sm:text-[11px] mt-1 text-right ${
      ...
      {/* Input Box */}
      <div className="p-2 sm:p-4 border-t bg-white flex items-center gap-2 sm:gap-3">
        <input
          value={input}
          onChange={(e) => setInput(e.target.value)}
          placeholder="Type a message..."
          className="flex-1 border border-gray-300 rounded-full px-3 py-2 sm:py-2.5 focus:outline-none focus:ring-2 focus:ring-green-500 text-sm sm:text-base"
